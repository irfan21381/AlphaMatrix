...
import random
from typing import Any, Optional   # ✅ FIX ADDED
...
THRESHOLDS = {
    "thermal_throttling": {"cpu_temp": 70.0,  "cpu_load": 60.0},
    "battery_endurance":  {"battery_drain_rate": 5.0},
    "process_deadlock":   {"deadlock_score": 10.0},
}
...
class AlphaMatrixEnv:
    ...
    def reset(self, task: Optional[str] = None) -> dict:   # ✅ FIX HERE
        if task is not None:
            if task not in TASKS:
    ...
    def _init_battery(self):
        self._state = {
            ...
            "battery_drain_rate": random.uniform(20.0, 40.0),
            "screen_brightness":  random.uniform(70.0, 100.0),
            "active_radios":      random.uniform(3.0, 5.0),
        }
    ...
    def _apply_battery(self, action: str):
        ...
        s["battery_level"]      -= s["battery_drain_rate"] / 60.0
        s["battery_drain_rate"] += random.uniform(-1.0, 1.5)
